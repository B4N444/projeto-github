...
public class DAOTarefa extends ConexaoSQL {
    ...
    public int salvarTarefaDAO(Tarefa pTarefa) {
        try {
            ...
            conn = ConexaoSQL.conectar();//tenta conectar o banco
            ...
            try (PreparedStatement pstm = criarPreparedStatement(sql, Statement.RETURN_GENERATED_KEYS)) {
                pstm.setString(1, pTarefa.getTitulo());
    ...
    public ArrayList<Tarefa> listarTarefaDAO() {
        ...
        try {
            ...
            try (PreparedStatement pstm = criarPreparedStatement(sql, 0); ResultSet rs = pstm.executeQuery();) {
                while (rs.next()) {
                    Tarefa pTarefa = new Tarefa();
                    pTarefa.setId(rs.getInt("id"));
                    pTarefa.setTitulo(rs.getString("tarefas"));
    ...
    public boolean editarTarefaDAO(Tarefa pTarefa) {
        ...
        try {
            ...
            String sql = ("UPDATE tb_tarefas SET tarefas = ?,"
                    + "descricao = ?, data_tarefa = ?, hora_inicio = ?, hora_fim = ?, status = ?"
                    + " WHERE id = ? ");
    ...
    public ArrayList<LocalDate> listarDatasTarefasDAO() {
        ...
        try {
            conn = ConexaoSQL.conectar();
            String sql = "  SELECT DISTINCT data_tarefa FROM"
                    + " tb_tarefas ORDER BY data_tarefa";
    ...
    public ArrayList<Tarefa> listarTarefasDataDAO(LocalDate data) {
        ...
        try {
            ...
            String sql = "SELECT * FROM tb_tarefas WHERE data_tarefa = ? "
            ...
            try (PreparedStatement pstm = conn.prepareStatement(sql)) {
                ...
                try (ResultSet rs = pstm.executeQuery()) {
                    while (rs.next()) {
                        ...
                        t.setTitulo(rs.getString("tarefas"));
    ...
    ArrayList<Tarefa> lista = new ArrayList<>();
        ...
        try {
            ...
            String sql = "SELECT * FROM tb_tarefas WHERE data_tarefa BETWEEN ? AND ? ORDER BY"
                    + " data_tarefa, hora_inicio";
            ...
            try (PreparedStatement pstm = conn.prepareStatement(sql)) {
                ...
                try (ResultSet rs = pstm.executeQuery()) {
                    while (rs.next()) {
                        ...
                        tarefa.setTitulo(rs.getString("tarefas"));
